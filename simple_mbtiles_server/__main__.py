...
def _matches_poi_category(props, category):
    ...
    if not f:
        return False
    return props.get("subclass") in f["subclass"] or props.get("class") in f["class"]
            ...
            if layer_name == "transportation_name":
                ...
                for rf in raw_features:
                    ...
                    if not alpine and not entries:
                        continue
                    for coords in _decode_mvt_geometry(
                        geometry, tile_x, tile_y, zoom, extent
                    ):
                        for point in coords:
...
def _routes_along_path(path_coords, edge_routes, min_edges=2, walkable_only=True):
    ...
    *walkable_only* keeps road route relations out of the report. A path can
    ...
    for i in range(len(path_coords) - 1):
        ...
        b = (
            int(round(path_coords[i + 1][0] * _E5)),
...
def _named_way_warnings(
    path_coords, named_ways, tolerance_km=0.03, tagged_indices=None
):
    """
...
def _snap_pair(graph, from_lon, from_lat, to_lon, to_lat, max_snap_km=0.5):
    ...
    if best is None:
        ...
        return (
            start_node,
            exact(start_node, from_lon, from_lat),
            end_node,
            exact(end_node, to_lon, to_lat),
        )

    return (
        best[1],
        exact(best[1], from_lon, from_lat),
        best[2],
        exact(best[2], to_lon, to_lat),
    )
...
def simple_mbtiles_server(
    ...
    http_client = exit_stack.enter_context(
        httpx.Client(limits=httpx.Limits(max_connections=500))
    )
    ...
    if mbtiles:
        ...
        if first_mbtiles_url:
            contours_path = os.path.join(
                os.path.dirname(first_mbtiles_url), "contours.mbtiles"
            )
    ...
    fonts_dict = {("fonts-gl", "1.0.0"): extract("vendor/fonts-gl@1.0.0/fonts.tar.gz")}
    ...
    tile_data_features = {
        "sac_scale": False,
        "via_ferrata_scale": False,
        "trail_visibility": False,
        "smoothness": False,
    }
    ...
    def get_tile(identifier, version, z, x, y):
        ...
        allows_gzip = "gzip" in request.headers.get("accept-encoding", "").replace(
            " ", ""
        ).split(",")
    ...
    def get_styles(identifier, version):
        ...
        if contours_dict:
            style_dict["sources"]["contours"] = {
                "type": "vector",
                "tiles": [request.url_root + "v1/tiles/contours@1.0.0/{z}/{x}/{y}.mvt"],
                "minzoom": 0,
                "maxzoom": 14,
            }
            ...
            style_dict["layers"].extend(
                [contour_line_minor, contour_line_major, contour_label_layer]
            )

        if "sprite" in style_dict:
            style_dict["sprite"] = (
                request.url_root + "v1/styles/" + identifier + "@" + version + "/sprite"
            )
    ...
    def get_sprite_2x_json(identifier, version):
        return get_sprite_file(
            identifier, version, "sprite@2x.json", "application/json"
        )
        ...
        for z, x, y in tiles:
            ...
            if cached is None:
                ...
                if blob is None and z == 14:
                    ...
                    cached = (
                        _extract_contour_lines(blob, x // 2, y // 2, 13) if blob else []
                    )
                else:
    ...
    def compute_route(
        ...
        if len(tiles) > route_max_tiles:
            raise RoutingError(
                ...
                "insert a waypoint" % (len(tiles), route_max_tiles),
                400,
            )
        ...
        if prefer_routes or follow_route:
            ...
            if follow_route and not any(
                _route_matches(entry, follow_route) for _lon, _lat, entry in route_ways
            ):
                raise RoutingError(
        ...
        if not graph:
            ...
            if max_sac_scale or not allow_via_ferrata:
                limit_note = (
                    " with the requested difficulty limit "
                    "(max_sac_scale=%s, allow_via_ferrata=%s)"
                    % (max_sac_scale, allow_via_ferrata)
                )
            raise RoutingError(
                'no way passable for profile "%s"%s in area' % (profile, limit_note),
                404,
            )
        ...
        if start_snap_km > 0.5:
            raise RoutingError(
                ...
                % (from_lat, from_lon, start_snap_km * 1000),
                404,
            )
        if end_snap_km > 0.5:
            raise RoutingError(
                "no routable way within 500 m of destination %.5f,%.5f (closest: %.0f m)"
                % (to_lat, to_lon, end_snap_km * 1000),
                404,
            )
        ...
        if path_coords is None:
            raise RoutingError(
                ...
                "waypoint or a larger buffer_km" % (from_lat, from_lon, to_lat, to_lon),
                404,
            )
    ...
    def compute_elevation(coords):
        ...
        if len(tiles) > route_max_tiles:
            raise RoutingError(
                "area too large (%d tiles, limit %d)" % (len(tiles), route_max_tiles),
                400,
            )
    ...
    def search_poi(identifier, version, lat, lon, category, radius_km=15.0, limit=None):
        ...
        if category not in _POI_CATEGORY_FILTERS:
            raise RoutingError(
                ...
                % (category, ", ".join(sorted(_POI_CATEGORY_FILTERS))),
                400,
            )
    ...
    def _json(payload, status=200):
        return Response(
            status=status, content_type="application/json", response=json.dumps(payload)
        )
    ...
    def get_route(identifier, version):
            ...
            return _json(
                {
                    ...
                    "from=lat,lon&to=lat,lon&profile=foot|bike"
                },
                400,
            )

        profile = request.args.get("profile", "foot")
        with_elevation = request.args.get("elevation", "").lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        prefer_routes = request.args.get("prefer_routes", "").lower() in (
            "1",
            "true",
            "yes",
        )
    ...
    default_tiles = (
        (mbtiles[0]["IDENTIFIER"], mbtiles[0]["VERSION"]) if mbtiles else (None, None)
    )
    ...
    def _latlon(value, label):
        ...
        if not -90.0 <= lat <= 90.0 or not -180.0 <= lon <= 180.0:
            raise ToolError(
                "%s out of range: %s (expected [lat, lon])" % (label, value)
            )
        return lat, lon
    ...
    def _photon_get(path, params):
        ...
        if resp.status_code != 200:
            raise ToolError(
                "Photon server at %s returned HTTP %d" % (photon_url, resp.status_code)
            )
        try:
            return resp.json()
        except ValueError:
            raise ToolError(
                "Photon server at %s returned a non-JSON response" % photon_url
            )
    ...
    def tool_search_poi(args):
        ...
        if not isinstance(category, str):
            raise ToolError(
                "category is required; available: %s"
        ...
        try:
            features = search_poi(
                identifier, version, lat, lon, category, radius_km, limit
            )
        except RoutingError as exc:
    ...
    def tool_plan_route(args):
        ...
        waypoints = [
            _latlon(wp, "waypoints[%d]" % i) for i, wp in enumerate(raw_waypoints)
        ]
        ...
        if routes_used:
            result["routes"] = sorted(
                routes_used.values(), key=lambda r: r.get("segments", 0), reverse=True
            )[:12]
        if follow_route:
        ...
        if ascent is not None:
            ...
            result["duration_note"] = (
                "DIN 33466 estimate (300 m ascent / 500 m descent per hour)"
            )
        else:
            result["duration_note"] = (
                "flat estimate at %.1f km/h, no elevation surcharge"
    ...
    def tool_export_gpx(args):
        ...
        if not isinstance(raw_coords, (list, tuple)) or len(raw_coords) < 2:
            raise ToolError(
                "coordinates must be a list of at least two [lon, lat] pairs"
            )
        ...
        profile = args.get("profile") or "foot"
        gpx_id = gpx_store.write(
            to_gpx(coords, name=name, wpts=wpts, elevations=elevations)
        )
    ...
    mcp_server.tool(
        ...
        "waypoint instead of retrying blindly." % (route_max_crow_km, _SAFETY_NOTE),
        {
            "type": "object",
            "properties": {
                "waypoints": {
                ...
                "name": {
                    "type": "string",
                    "description": "track name used in the GPX file",
                },
                "buffer_km": {
                    "type": "number",
                    "minimum": 0.5,
    ...
    app.add_url_rule(
        "/v1/static/<string:identifier>@<string:version>/<string:file>",
        view_func=get_static,
    )
